...
void Commands::privmsg_command(Serv &server, Client &user, std::string command)
{
    ...
    {
        std::string ctcpMessage = message.substr(1, message.size() - 2); // Enlever les caractères \x01

        if (ctcpMessage.substr(0, 4) == "DCC ")
        {
...
void Commands::handleDCC(Serv &server, Client &user, const std::string &target, const std::string &ctcpMessage)
{
    ...
    std::string relayMessage = ":" + user.getNickname() + " PRIVMSG " + target + " :\x01" + ctcpMessage + "\x01\r\n";
